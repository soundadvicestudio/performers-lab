...
export default async function handler(req, res) {
  ...
  return res.status(400).json({ error: 'Missing or invalid type. Must be "reminder" or "morning".' });
}
...
async function handleReminder(req, res) {
  try {
    const now = new Date();
    const windowStart = new Date(now.getTime() + 23 * 3600 * 1000).toISOString();
    const windowEnd   = new Date(now.getTime() + 25 * 3600 * 1000).toISOString();

    const { data: events } = await supabaseRequest(
      'GET',
      `/rest/v1/events?starts_at=gte.${encodeURIComponent(windowStart)}&starts_at=lte.${encodeURIComponent(windowEnd)}&reminder_sent=eq.false&status=eq.upcoming&select=*`
    );

    if (!events?.length) {
      return res.status(200).json({ message: 'No events in window' });
    }
    ...
    for (const event of events) {
      // Fetch all RSVPed members for this event
      const { data: rsvps } = await supabaseRequest(
        'GET',
        `/rest/v1/event_rsvps?event_id=eq.${event.id}&select=user_id`
      );

      if (!rsvps?.length) {
        await supabaseRequest('PATCH', `/rest/v1/events?id=eq.${event.id}`, { reminder_sent: true });
        eventsProcessed++;
        continue;
      }
      ...
      for (const rsvp of rsvps) {
        ...
        try {
          await supabaseRequest('POST', '/rest/v1/notifications', {
            ...
            type: 'event_reminder',
            title: `Session reminder: ${event.title}`,
            body: 'Your session starts in 24 hours. See you there.',
            link: `/app/event.html?id=${event.id}`,
            read: false,
          });
          totalMembersNotified++;
        } catch (err) {
          console.error(`Failed to send notification to ${member.user_id}:`, err);
        }
        ...
        if (member.email_notify_events !== false) {
          try {
            ...
            console.error(`Failed to send reminder email to ${member.user_id}:`, err);
          }
        }
      }
    ...
    console.error('sendNotifications reminder error:', err);
...
async function handleMorning(req, res) {
  try {
    const now = new Date();
    ...
    console.error('sendNotifications morning error:', err);
    return res.status(500).json({ error: 'Internal server error' });
  }
}
...
function buildReminderEmail(displayName, eventTitle, ctTime, localLine, eventId) {
  const viewUrl = `https://performers-lab.com/app/event.html?id=${eventId}`;
  return `<!DOCTYPE html>
<html lang="en">
  ...
  <tr><td align="center">
    <table width="100%" style="max-width:600px;background:#0d0d0d;border-radius:8px;border:1px solid rgba(201,169,110,0.28);overflow:hidden;">
      ...
      <tr><td style="padding:24px 36px 16px;border-bottom:1px solid rgba(240,237,230,0.08);">
        ...
        <h2 style="margin:8px 0 0;font-family:Georgia,serif;font-size:22px;font-weight:400;color:#c9a96e;">See you tomorrow, ${esc(displayName)}.</h2>
      </td></tr>
      <tr><td style="padding:28px 36px;font-size:15px;color:#f0ede6;line-height:1.75;font-weight:300;">
        <p style="margin:0 0 12px;"><strong style="color:#f0ede6;">${esc(eventTitle)}</strong> starts tomorrow at ${esc(ctTime)}.</p>
        ${localLine ? `<p style="margin:0 0 12px;color:rgba(240,237,230,0.58);font-size:14px;">${esc(localLine)}</p>` : ''}
        <p style="margin:0;">Log in to The Performer's Lab to get ready for the session.</p>
      </td></tr>
      <tr><td style="padding:0 36px 28px;">
        <table cellpadding="0" cellspacing="0"><tr><td>
          <a href="${viewUrl}" style="display:inline-block;background:#c9a96e;color:#070707;padding:12px 28px;border-radius:2px;font-size:11px;font-weight:700;letter-spacing:0.16em;text-transform:uppercase;text-decoration:none;">View Session →</a>
        </td></tr></table>
      </td></tr>
